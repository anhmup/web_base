...
class mess_passing_H(nn.Module):
    ...
    def forward(self, x):
        ...
        for cnt in range(2,h):
            feature =  self.t2d_layers(feature_list_old[cnt-1])
...
class bottel_neck(nn.Module):
    def __init__(self, in_channels = 512, out_channels = 512):
        super(bottel_neck, self).__init__()
...
class updasample_combines(nn.Module):
    def __init__(self ):
        ...
        self.conv = nn.Conv2d(64, 256, kernel_size=3, stride=1, padding= 'same')
...
class model_split(nn.Module):
    def __init__(self):
        ...
        # row branch
        self.bridge_4_row = bottel_neck()
        self.LR_row_layer = mess_passing_W(in_channels=512, out_channels=512)
        self.up_4_3_row = up_blocks(512, 256)
        self.bridge_3_row = bottel_neck(in_channels=256, out_channels=256)
        self.up_3_2_row = up_blocks(256, 128)
        self.bridge_2_row = bottel_neck(in_channels=128, out_channels=128)
        self.up_2_1_row = up_blocks(128, 64)
        self.bridge_1_row = bottel_neck(in_channels=64, out_channels=64)

        #col branch
        self.bridge_4_col = bottel_neck()
        self.TD_col_layer = mess_passing_H(in_channels=512, out_channels=512)
        self.up_4_3_col = up_blocks(512, 256)
        self.bridge_3_col = bottel_neck(in_channels=256, out_channels=256)
        self.up_3_2_col = up_blocks(256, 128)
        self.bridge_2_col = bottel_neck(in_channels=128, out_channels=128)
        self.up_2_1_col = up_blocks(128, 64)
        self.bridge_1_col = bottel_neck(in_channels=64, out_channels=64)
    ...
    def forward(self, image):
        ...
        # row _branch
        p4_row = self.bridge_4_row(c4)
        p4_LR_row = self.LR_row_layer(p4_row)
        c3_bridge_row = self.bridge_3_row(c3)
        p3_row = self.up_4_3_row(p4_LR_row, c3_bridge_row)
        c2_bridge_row = self.bridge_2_row(c2)
        p2_row = self.up_3_2_row(p3_row, c2_bridge_row)
        c1_bridge_row = self.bridge_1_row(c1)
        p1_row = self.up_2_1_row(p2_row, c1_bridge_row)
        #col branch
        p4_col = self.bridge_4_col(c4)
        p4_TD_col = self.TD_col_layer(p4_col)
        c3_bridge_col = self.bridge_3_col(c3)
        p3_col = self.up_4_3_col(p4_TD_col, c3_bridge_col)
        c2_bridge_col = self.bridge_2_col(c2)
        p2_col = self.up_3_2_col(p3_col, c2_bridge_col)
        c1_bridge_col = self.bridge_1_col(c1)
        p1_col = self.up_2_1_col(p2_col, c1_bridge_col)

        # head
        row_feature = self.updasample_combines_W(p1_row)
        final_row = self.predict_row(row_feature)

        col_feature = self.updasample_combines_H(p1_col)
